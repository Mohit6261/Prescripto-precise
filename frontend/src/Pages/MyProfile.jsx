    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
     {
      ...
      ?<label htmlFor='image'>
         <div className='inline-block relative cursor-pointer'>
          <img className='w-36 rounded opacity-35' src={image ? URL.createObjectURL(image):userData.image} alt="" />
      ...
      <div>
        ...
        isEdit
          ? <select className='max=w=20 bg-gray-100' onChange={(e)=>setUserData(prev=>({...prev,gender:e.target.value}))} value={userData.gender}>
             ...
             <option valur="Female">Female</option>
          </select>
          : <p>{userData.gender}</p>
      }
      ...
      <div className='mt-10 '>
        {
          isEdit
          ? <button   className='border border-primary px-8 py-2 rounded-full hover:bg-primary'onClick={updateUserProfileData}>Save Information</button>
          : <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary' onClick={()=>setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  )
